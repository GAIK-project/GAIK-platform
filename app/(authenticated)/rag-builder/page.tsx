...
export default function Home() {
  ...
  const [persistantAssistantName, setPersistantAssistantName] =
    useState<string>("");
  ...
  const handleSubmit = async () => {
    setStatus("loading");
    scrollToBottom();
    const check: boolean = await checkUniqueName(assistantName);
    if (check) {
      setErrorMessage("Assistant name already exists, please select a new one");
      return;
    }
    const validationCheck: boolean = validateForm(systemPrompt, links);
    if (!validationCheck) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setStatus("success");
        const newAssistantName: string = sanitizeTableName(assistantName);
    ...
    <div className="container">
      ...
      <div className="welcome-section faded-shadow">
        ...
        <div className="welcome-subtitle">
          Here you can add custom data from weblinks or files as context, so
          that we can build a RAG asssistant to fetch answers from the given
          context!
        </div>
      </div>
      ...
      <div className="centered-section">
        <div className="section">
          ...
          <AutoGrowingTextarea
            ...
            placeholder="You are an AI assistant who knows everything about Formula One."
          />
        </div>
        ...
        <div className="section">
          ...
          <FileUpload
            files={files}
            setFiles={setFiles}
            runValidation={validateAfterFileChange}
          />
        </div>
          ...
          {status === "success" && (
            <>
              <p className="text-green-600 text-lg mb-4">
                {message} Go to dataset manager here to see the progress on your
                datasets.
              </p>
              <button
                onClick={handleNext}
          ...
          {status === "error" && (
            <>
              <p className="text-red-600 text-lg mb-4">
                {message} Reload the page here or try again later.
              </p>
              <button
                onClick={handleRetry}
