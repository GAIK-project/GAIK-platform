...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!assistantId || typeof assistantId !== 'string') {
            return NextResponse.json({ error: 'Missing or invalid assistantId parameter' }, { status: 400 });
        }

        // Sanitize input (if you want to sanitize IDs too)
        const safeAssistantId = sanitizeTableName(assistantId);
        if (!safeAssistantId) {
            return NextResponse.json({ error: 'Invalid assistantId after sanitization' }, { status: 400 });
        }
        ...
        return NextResponse.json({ message: "Assistant retrieved", data: result[0] }, { status: 200 });
    } catch (error) {
