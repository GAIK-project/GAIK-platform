    ...
    if (links[0]) {
      for await (const url of links) {
        ...
        if (!content) {
          errors.push({
            type: "scrape",
            details: `Failed to scrape URL: ${url}`,
          });
          continue;
        }
    ...
    if (files) {
      ...
      for (const file of files) {
        ...
        if (totalSize > MAX_TOTAL_SIZE_BYTES) {
          errors.push({
            type: "sizeLimit",
            details: `File exceeds total size limit: ${file.name}`,
          });
          continue;
        }
        ...
        if (!category) {
          errors.push({
            ...
            details: `Unsupported file type (${file.type}) for file: ${file.name}`,
          });
          continue;
        }
        ...
        switch (category) {
          ...
          default:
            errors.push({
              ...
              details: `Unknown category (${category}) for file: ${file.name}`,
            });
            continue;
        }
