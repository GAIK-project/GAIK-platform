...
export async function POST(request: NextRequest) {
    try {
      ...
      // Insert new assistant metadata row
      const [assistantRow] = await db
        ...
        .returning(); // Capture inserted row (for id)
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        // Update currentChunk after each insert
        await db
          .update(assistants)
          .set({
            currentChunk: i + 1,
            taskCompleted: i + 1 === chunks.length, // mark complete when done
          })
