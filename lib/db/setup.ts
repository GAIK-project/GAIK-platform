...
async function getSupabaseCredentials() {
  ...
  console.log('Go to Project Home Screen -> Click \'Connect\' and Select App Frameworks tab to find your credentials\n');
...
async function getCSCCredentials() {
  ...
  console.log('- https://docs.csc.fi/support/faq/how-to-get-Allas-s3-credentials/\n');

  const accessKeyId = await question('Enter your Allas Access Key ID: ');
  const secretAccessKey = await question('Enter your Allas Secret Access Key: ');
...
async function getOpenAIKey() {
  ...
  console.log('Please visit: https://platform.openai.com/api-keys to get your API key\n');
...
async function main() {
  ...
  console.log('For each step, you can choose to skip if you don\'t need it now.\n');
  console.log('❗ Warning running this script will overwrite your existing .env.local file ❗\n');
  ...
  try {
    ...
    console.log('\nNEXT STEPS:');
    console.log('1. Run your database migrations (if using Supabase/Drizzle) "pnpm db:push" OR "pnpm db:migrate"');
