...
export default function DatasetCard({ assistant, onEdit, onDelete }: Props) {
  ...
  return (
    <div className={styles.card}>
      ...
      <p><strong>Chunks:</strong> {assistant.currentChunk} / {assistant.totalChunks}</p>
      <p><strong>Created:</strong> {new Date(assistant.createdAt).toLocaleString()}</p>
      <p><strong>Completed:</strong> {assistant.taskCompleted ? "Yes" : "No"}</p>
      ...
      <div className={styles.actions}>
        <button className={styles.button} onClick={handleEdit}>Save Edits</button>
        <button className={styles.button} onClick={() => onDelete(assistant.id)}>Delete Dataset</button>
        <button className={styles.button} onClick={() => alert("This feature is coming soon")}>Add Data</button>
      </div>
    </div>
  );
}
